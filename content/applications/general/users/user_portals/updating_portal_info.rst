...
last used. They can be renamed by clicking their :icon:`fa-pencil` :guilabel:`edit` button or
deleted by clicking their :icon:`fa-trash` :guilabel:`Delete` button.
...
the Odoo database. A database user with administrator access rights can make any necessary changes
by clicking into the field and updating the user's email address.
